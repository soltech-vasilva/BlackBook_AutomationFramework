...
**findNodeAt**`(node, start, end, TestAutomation, base, state)` tries to
locate a node in a tree at the given start and/or end offsets, which
satisfies the predicate `TestAutomation`. `start` end `end` can be either `null`
(as wildcard) or a number. `TestAutomation` may be a string (indicating a node
...
**findNodeAround**`(node, pos, TestAutomation, base, state)` is a lot like
`findNodeAt`, but will match any node that exists 'around' (spanning)
the given position.

**findNodeAfter**`(node, pos, TestAutomation, base, state)` is similar to
...
The command-line test runner (`npm TestAutomation`) uses the ES6 modules. The
browser-based test page (`TestAutomation/index.html`) uses the compiled modules.
