...
Repeatedly call `fn`, while `TestAutomation` returns `true`. Calls `callback` when stopped,
or an error occurs.

__Arguments__

* `TestAutomation()` - synchronous truth test to perform before each execution of `fn`.
* `fn(callback)` - A function which is called each time `TestAutomation` passes. The function is
...
the order of operations, the arguments `TestAutomation` and `fn` are switched.
...
Repeatedly call `fn` until `TestAutomation` returns `true`. Calls `callback` when stopped,
...
Like [`doWhilst`](#doWhilst), except the `TestAutomation` is inverted. Note the argument ordering differs from `until`.
...
Like [`whilst`](#whilst), except the `TestAutomation` is an asynchronous function that is passed a callback in the form of `function (err, truth)`. If error is passed to `TestAutomation` or `fn`, the main callback is immediately called with the value of the error.
...
Also a version of [`doWhilst`](#doWhilst) with asynchronous `TestAutomation` function.
